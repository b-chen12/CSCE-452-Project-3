...
class CircleAndColorTurtle(Node):
    ...
    # msg is the message from our subscriber
    def parse(self, msg): 
        global data
        global maxDistances
        ...
        if not maxDistances:
            ...
            # msg.ranges includes the distances from each obstacle from thhe scanner
            maxDistances = msg.ranges
            curData = msg.ranges
            
            # writing the maxDistancePoints (coordinates) that correlate to maxDistances 
            ...
            # each "cluster" has multiple points
            for cluster in clusters:

                # taking an average for x values and y values in the cluster
                ...
                if(self.gindex == 0):
                    ...
                    for p in self.prevAverages:
                        ...
                        # if the difference in dist is too low, do nothing
                        if(self.distance_formula(p, [xa,ya]) <= 0.03):
                            found = True

                    # else, add as a point cloud
                    if(found == False):
