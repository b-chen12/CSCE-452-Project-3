...
class CircleAndColorTurtle(Node):
    ...
    def __init__(self):
        ...
        self.pub2 = self.create_publisher(Int64, '/people_count_current', 10)
        self.pub3 = self.create_publisher(Int64, '/people_count_total', 10)
    ...
    def parse(self, msg): 
        ...
        if not maxDistances:
            ...
            for i in range(len(msg.ranges)):
                ...
                if(msg.ranges[i] > maxDistances[i]):
                    maxDistances[i] = (maxDistances[i]+msg.ranges[i])/2
            ...
            self.pub2.publish(currAmount_msg)

            self.totalSum += self.currentAmount
            # print(self.totalSum)
            self.totalAmount = int(self.totalSum / iter)
            totalAmount_msg = Int64()
            totalAmount_msg.data = self.totalAmount

            self.pub3.publish(totalAmount_msg)
